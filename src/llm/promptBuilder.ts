...
export function buildJsonSystemPrompt(botName: string, tools: ToolDef[]): string {
  ...
  return [
    ...
    `- "craft/make X" -> craftItem (it gathers a table itself if one is nearby, but NOT raw`,
    `  materials — if X clearly needs something you don't have yet, e.g. planks/sticks for a`,
